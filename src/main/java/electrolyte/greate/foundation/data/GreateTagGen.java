...
public class GreateTagGen {
    ...
    private static final TagKey<Item> DUSTS_NETHERRACK = ForgeRegistries.ITEMS.tags().createTagKey(new ResourceLocation("forge", "dusts/netherrack"));
        ...
        protected void addTags(Provider pProvider) {
            for(int i = 0; i < TM.length; i++) {
                this.tag(HIDE_FROM_RECIPE_VIEWERS)
                        ...
                        .add(AllItems.POWDERED_OBSIDIAN.asItem())
                        .add(AllItems.WHEAT_FLOUR.asItem());
            }
            this.tag(DUSTS_NETHERRACK).add(AllItems.CINDER_FLOUR.asItem());
        }
    }
}
